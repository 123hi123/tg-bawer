...
type resolvedGoogleService struct {
	OwnerUserID int64
	Config      gemini.ServiceConfig
}

type resolvedGrokService struct {
	OwnerUserID int64
	Client      *grok.Client
}
...
` + "`/service add grok custom my-grok2 http://your-grok-host:8080 sk-...`"
...
func (b *Bot) cmdServiceAdd(msg *tgbotapi.Message, args []string) {
	...
	case "standard", "gemini", "origin", "original":
		...
		if err := b.ensurePrivateServiceQuota(msg.From.ID, gemini.ServiceTypeStandard); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
		if err := validateServiceDefinition(gemini.ServiceTypeStandard, args[2], args[3], "", "", "", ""); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
		...
		if err != nil {
			if isDuplicateServiceNameErr(err) {
				b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ 服務名稱重複，同一個帳號不能新增同名服務"))
				return
			}
	...
	case "custom":
		...
		if err := b.ensurePrivateServiceQuota(msg.From.ID, gemini.ServiceTypeCustom); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
		if err := validateServiceDefinition(gemini.ServiceTypeCustom, args[2], args[4], args[3], "", "", ""); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
	...
	case "vertex":
		...
		if err := b.ensurePrivateServiceQuota(msg.From.ID, gemini.ServiceTypeVertex); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
		if err := validateServiceDefinition(gemini.ServiceTypeVertex, args[2], args[3], baseURL, projectID, location, model); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
	...
	case "grok":
		...
		if err := b.ensurePrivateServiceQuota(msg.From.ID, grok.ServiceTypeGrok); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
		if err := validateServiceDefinition(grok.ServiceTypeGrok, name, apiKey, baseURL, "", "", ""); err != nil {
			b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ "+err.Error()))
			return
		}
...
func (b *Bot) cmdServiceUseDeprecated(msg *tgbotapi.Message) {
	b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "⚠️ `/service use <服務ID>` 已廢棄，現在不需要使用這個指令。"))
}
...
func (b *Bot) cmdServicePublic(msg *tgbotapi.Message, args []string) {
	...
	isPublic := action == "pub"
	if isPublic && !b.isAdminUser(msg.From.ID) {
		b.api.Send(tgbotapi.NewMessage(msg.Chat.ID, "❌ 只有管理員可以將服務設為公開"))
		return
	}
...
func (b *Bot) resolveAllServiceConfigs(userID int64) ([]resolvedGoogleService, error) {
	services, err := b.db.GetAllUserServices(userID)
	if err != nil {
		return nil, err
	}

	var configs []resolvedGoogleService
	for _, service := range services {
		...
		configs = append(configs, resolvedGoogleService{
...
func (b *Bot) resolveGrokService(userID int64) *resolvedGrokService {
	services, err := b.db.GetAllUserServices(userID)
	if err == nil {
		for _, s := range services {
			if s.Type == grok.ServiceTypeGrok {
				// Empty strings for imgModel/editModel/videoModel use the package defaults.
				return &resolvedGrokService{
					OwnerUserID: s.UserID,
					Client:      grok.NewClient(s.APIKey, s.BaseURL, "", "", ""),
				}
			}
		}
	}
...
func (b *Bot) resolveGrokClient(userID int64) *grok.Client {
	service := b.resolveGrokService(userID)
	if service == nil {
		return nil
	}
...
func (b *Bot) resolveServiceConfig(userID int64) (gemini.ServiceConfig, string, error) {
	...
	if len(services) > 0 {
		return services[0].Config, services[0].Config.Name, nil
	}
