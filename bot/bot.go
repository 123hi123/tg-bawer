...
func (b *Bot) handleMessage(msg *tgbotapi.Message) {
	if msg.From != nil && b.rejectWhenMaintaining(msg) {
		return
	}
...
/state - 查看專案磁碟使用狀態
/queue - 查看自己的待重試數量
/record - 查看模型統計（管理員看全域，其他人看自己的服務）
/addprompt - 為過往回覆補上提示詞
/help - 顯示幫助`

	if b.adminUserID != 0 && msg.From != nil && msg.From.ID == b.adminUserID {
		text += "\n/errors - 查看最近錯誤記錄（管理員）"
		text += "\n/fix database - 檢查並修復資料庫（管理員）"
	}
...
func (b *Bot) registerCommands() {
	commands := tgbotapi.NewSetMyCommands(
		...
		tgbotapi.BotCommand{Command: "errors", Description: "查看最近的錯誤記錄（管理員）"},
		tgbotapi.BotCommand{Command: "state", Description: "查看磁碟、重試佇列與資源使用狀態"},
		tgbotapi.BotCommand{Command: "queue", Description: "查看自己的待重試任務數量"},
		tgbotapi.BotCommand{Command: "record", Description: "查看模型統計（管理員看全域，其他人看自己的服務）"},
		tgbotapi.BotCommand{Command: "fix", Description: "檢查並修復資料庫（管理員）"},
...
func (b *Bot) handleCallback(callback *tgbotapi.CallbackQuery) {
	if callback.From != nil && b.rejectCallbackWhenMaintaining(callback) {
		return
	}
...
func (b *Bot) handleTextMessage(msg *tgbotapi.Message) {
	...
	if len(allServices) == 0 && !b.hasAvailableService(msg.From.ID) {
...
func (b *Bot) handlePhoto(msg *tgbotapi.Message) {
	...
	if len(allServices) == 1 {
		serviceName = allServices[0].Config.Name
	}
	...
	var lastErr error
	googleSuccess := false
	resultTarget := modelStatTarget{}
	if len(allServices) > 0 {
		resultTarget = buildGoogleStatTarget(allServices[0].OwnerUserID, allServices[0].Config.Model)
		b.recordModelUsage(resultTarget)
	}
	...
	for svcIdx, svcCfg := range allServices {
		...
		for attempt := 0; attempt < 6; attempt++ {
			b.updateMessage(processingMsg, fmt.Sprintf("⏳ 處理中... (服務 %d/%d，嘗試 %d/6)\n🔌 服務: %s\n📐 圖片: %dx%d\n📏 比例: %s",
				svcIdx+1, len(allServices), attempt+1, svcCfg.Config.Name, imageInfo.Width, imageInfo.Height, ratioInfo))
			...
			if lastErr == nil && result != nil && len(result.ImageData) > 0 {
				resultTarget = buildGoogleStatTarget(svcCfg.OwnerUserID, svcCfg.Config.Model)
				googleSuccess = true
				break
			}
	...
	if !googleSuccess && grokService != nil {
		resultTarget = buildGrokStatTarget(grokService.OwnerUserID, grokService.Client.EditModel())
		b.recordModelUsage(resultTarget)
		...
		for attempt := 0; attempt < 6; attempt++ {
			...
			grokResult, grokErr := grokService.Client.EditImage(ctx, imgData, prompt, "1024x1024")
	...
	if result == nil || len(result.ImageData) == 0 {
		source := "google"
		target := modelStatTarget{}
		if len(allServices) > 0 {
			target = buildGoogleStatTarget(allServices[0].OwnerUserID, allServices[0].Config.Model)
		}
		if grokService != nil {
			source = "grok"
			target = buildGrokStatTarget(grokService.OwnerUserID, grokService.Client.EditModel())
		}
		var svcCfg gemini.ServiceConfig
		if len(allServices) > 0 {
			svcCfg = allServices[0].Config
		}
	...
	if withVoice && len(allServices) > 0 {
		ttsClient := gemini.NewClientWithService(allServices[0].Config)
