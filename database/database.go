...
func (d *Database) configureSQLite() error {
	...
	if _, err := d.db.Exec(fmt.Sprintf(`PRAGMA busy_timeout = %d`, sqliteBusyTimeoutMS)); err != nil {
		return err
	}
	return nil
}
...
func (d *Database) init() error {
	...
	if err := d.ensureModelRequestStatsTable(); err != nil {
		return err
	}
...
func (d *Database) UpdateFailedGenerationRetryError(id int64, lastError string) error {
	_, err := d.db.Exec(`
		UPDATE failed_generations
		SET last_error = ?,
		    last_retry_at = CURRENT_TIMESTAMP
